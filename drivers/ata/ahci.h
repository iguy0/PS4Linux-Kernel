...
enum { AHCI_MAX_PORTS = 32,
       ...
       PORT_IRQ_FREEZE = PORT_IRQ_HBUS_ERR | PORT_IRQ_IF_ERR |
...
struct ahci_host_priv {
	...
	int (*get_irq_vector)(struct ata_host *host, int port);
};
...
#define AHCI_SHT(drv_name)                                                     \
	ATA_NCQ_SHT(drv_name),                                                 \
		.can_queue = AHCI_MAX_CMDS, .sg_tablesize = AHCI_MAX_SG,       \
		.dma_boundary = AHCI_DMA_BOUNDARY,                             \
		.shost_attrs = ahci_shost_attrs, .sdev_attrs = ahci_sdev_attrs
...
int ahci_do_softreset(struct ata_link *link, unsigned int *class, int pmp,
...
static inline u32 __indword(u32 port)
{
	return 0;
}
...
static inline void bpcie_write_to_bar2_and_0x180000_and_offset(u32 offset,
							       u32 val)
{
...
static inline void bpcie_ahci_write(struct f_resource *r_mem, u32 offset,
				    u32 val)
{

...
static const struct pci_device_id ahci_pci_tbl[] = {
	...
	{ PCI_VDEVICE(INTEL, 0x9dd3),
	  board_ahci_mobile }, /* Cannon Lake PCH-LP AHCI */
	...
	{ PCI_VDEVICE(INTEL, 0x34d3),
	  board_ahci_mobile }, /* Ice Lake LP AHCI */
...
#endif
	if (using_dac && !dma_set_mask(&pdev->dev, DMA_BIT_MASK(64))) {
		rc = dma_set_coherent_mask(&pdev->dev, DMA_BIT_MASK(64));
		if (rc) {
			rc = dma_set_coherent_mask(&pdev->dev,
						   DMA_BIT_MASK(32));
			if (rc) {
				dev_err(&pdev->dev,
...
static int ahci_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	for (i = 0; i < host->n_ports; i++) {
		...
		ata_port_pbar_desc(ap, ahci_pci_bar, 0x100 + ap->port_no * 0x80,
				   "port");
...
*/
	...
	struct pci_dev *sc_dev = pci_get_device(
		PCI_VENDOR_ID_SONY, PCI_DEVICE_ID_SONY_BAIKAL_PCIE, NULL);
	if (!sc_dev) {
	...
	dev_info(
		dev,
		"Baikal SATA EFUSE VALUE: 0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n",
		v16, v17, 0, 0, 0, 0);
	r_mem = ctlr->r_mem;
	...
	bpcie_ahci_write(ctlr->r_mem, 0x20A0,
			 (v22 & 0xFBFF03FF) |
				 ((bpci_usb_ahci ? v423 : v422) << 10) |
				 0x4000000);
	...
	bpcie_ahci_write(ctlr->r_mem, 0x2054,
			 v36 & (0xFFFFF07F |
				((bpci_usb_ahci ? v421 : v420) << 7)));

	v42 = bpcie_ahci_read(ctlr->r_mem, 0x201C);
	bpcie_ahci_write(ctlr->r_mem, 0x201C, v42 | 4);

	v49 = bpcie_ahci_read(ctlr->r_mem, 0x2078);
	bpcie_ahci_write(ctlr->r_mem, 0x2078,
			 v49 & (0xFFFFFE0F |
				16 * ((bpci_usb_ahci ? v419 : ivar_out))));
	...
	if (bpcie_buffer) {
		...
		if (v57 <= 0x12)
			trace_length =
				v56 +
				((_QWORD)ctlr_->apcie_bpcie_buffer & 0x1F);
		...
		case 2u:
			...
			bpcie_ahci_write(ctlr->r_mem, 0x204C,
					 (v123 & 0xFFFFC0FF) | 0x1D00);

			v264 = bpcie_ahci_read(ctlr->r_mem, 0x2054);
			bpcie_ahci_write(ctlr->r_mem, 0x2054,
					 (v264 & 0xFFFF9FFF) | 0x4000);
